...
.macro display_game_over
    ...
    # s0 = health remaining (determines # of yellow stars and message)
    la $s0, player_info
    lw $s0, 16($s0)
    
    fail_screen: bgtz $s0, success_screen
        set_rect 44, 76, GAME_OVER_WIDTH, GAME_OVER_HEIGHT
        la $t5, gameover_hex_arr
        apply_rect colour_address_and_increment $t5
        j done_draw_screen
    success_screen:
        # TODO - display something else if they succeed
    ...
    draw_stars: bge $t6, $t7, done_draw_stars
        ...
        pick_gray_star: bgtz $s0, pick_yellow_star
